...
async def wygraj_rekrutacje(guild, channel, applicant_id, event_name):
    ...
    if applicant:
        try: await applicant.send(f"✅ Hej twoje podanie na event **{event_name.upper()}** zostało zaakceptowane i otrzymałeś rangę na serwerze Wilo-Eventy sprawdź kanały na dole serwera! !")
        except: pass
...
async def przegraj_rekrutacje(guild, channel, applicant_id, event_name):
    applicant = guild.get_member(applicant_id)
    if applicant:
        try: await applicant.send(f"❌ Hej niestety twoje podanie na event **{event_name.upper()}** zostało odrzucone powodzenia w przyszłości!")
        except: pass
...
class RecruitmentModal(ui.Modal):
    ...
    q3 = ui.TextInput(label='3. Wyjaśnij czym jest RP / Reakcja na Wila ', placeholder='Wyjaśnij czym jes RP? / Co zrobisz jak  spotkasz Wila?', style=discord.TextStyle.paragraph, required=True)
    ...
    q5 = ui.TextInput(label='5. Link do filmu z mikrofonem', placeholder='Link do filmu z mikrofonem i povem z mc ', style=discord.TextStyle.paragraph, required=True)
